...
router.get('/login', function(req, res, next) {
    res.render('reLogin.ejs', { LogedInUser: req.user ? req.user : '',CartQty: req.session.cart ? req.session.cart.totalQty : 0 } );// req.flash('loginMessage')//
});

router.get('/signup', function(req, res) {
    res.render('signup.ejs', { message:req.flash('signupMessage'),LogedInUser: req.user ? req.user : '',CartQty: req.session.cart ? req.session.cart.totalQty : 0  });//
});
...
router.get('/login1', function(req, res, next) {
    ...
    res.render('giftsPage',{LogedInUser: req.user ? req.user : '',CartQty: req.session.cart ? req.session.cart.totalQty : 0});//, {orders: orders_json,etitle : "present"});
});
...
router.get('/signin', function(req, res, next) {
    ...
    interest.find({},function(err,orders) {
        ...
        res.render('signUpPage', {orders: orders_json,etitle : "sign Up Page",LogedInUser: req.user ? req.user : '',CartQty: req.session.cart ? req.session.cart.totalQty : 0});
